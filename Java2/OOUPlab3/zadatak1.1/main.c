...
void animalPrintMenu(struct Animal *animal1){
  
   printf("%s voli %s\n",animal1->vtable[0](animal1),animal1->vtable[2]());
...
int main(int argc, char *argv[]){
  for (int i=0; i<argc/2; ++i){
    char* alloc="stack";
    struct Animal* p=(struct Animal*)myfactory(argv[1+2*i], argv[1+2*i+1],alloc);
